...
def obj_center(args, objX, objY, centerX, centerY):  # first process
	...
	while True:
		...
		# draw keypoints on frame (replace with blob drawing)
		if rect is not None:
			(x, y, w, h) = rect
		...
		cv2.imshow("Pan-Tilt blob Tracking", frame)
		cv2.waitKey(1)
...
def set_servos(pan, tlt):  # drives servos to specific angle values (-90-90)
	# signal trap to handle keyboard interrupt
	signal.signal(signal.SIGINT, signal_handler)
